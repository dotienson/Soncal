  ...
  return (
    <button
      ...
      onClick={(e) => {
        ...
        if (e.detail === 0) {
          // Keyboard triggered click usually has detail 0
          onClick();
        }
      }}
...
function getFractionText(numStr: string, isMl?: boolean): string | null {
  ...
  const fractionValStr = Number.isInteger(fractionVal)
    ? fractionVal.toString()
    : +fractionVal.toFixed(3);
...
export default function App() {
  const [weightStr, setWeightStr] = useState<string>(
    () => localStorage.getItem("medCalc_weight") || "",
  );
  ...
  const [isPremium, setIsPremium] = useState<boolean>(
    () => localStorage.getItem("medCalc_premium") === "true",
  );
  const [keepAwake, setKeepAwake] = useState<boolean>(
    () => localStorage.getItem("medCalc_keepAwake") !== "false",
  );
  const [premiumModalOpen, setPremiumModalOpen] = useState(false);
  const [premiumCode, setPremiumCode] = useState("");
  const [premiumError, setPremiumError] = useState(false);

  const [askPinModalType, setAskPinModalType] = useState<
    "create" | "verify_edit" | "verify_setting" | "change_pin" | null
  >(null);
  const [secretPin, setSecretPin] = useState<string>(
    () => localStorage.getItem("medCalc_secretPin") || "",
  );
  const [hasPromptedPin, setHasPromptedPin] = useState<boolean>(
    () => localStorage.getItem("medCalc_hasPromptedPin") === "true",
  );
    ...
    const requestWakeLock = async () => {
      try {
        if (
          ...
          if (!nativeWakeLock) {
            nativeWakeLock = await (navigator as any).wakeLock.request(
              "screen",
            );
  ...
  const [theme, setTheme] = useState<Theme>(
    () => (localStorage.getItem("medCalc_theme") as Theme) || "slate",
  );
  ...
  const renderColoredExpression = (expression: string) => {
    ...
    return formattedStr.split("").map((char, i) => {
      if (char === ",") {
        return (
          <span key={i} className="text-[0.6em] ml-[1px] mr-[2px] opacity-70">
            ,
          </span>
        ); // Make comma look nice
      }
      if (["+", "-", "×", ":"].includes(char)) {
        if (i === expression.length - 1) {
          return (
            <span
              key={i}
              className="inline-flex items-center justify-center bg-slate-800 text-white shadow-inner text-[0.3em] align-top ml-[4px] mt-[10px] font-black w-[1.3em] h-[1.3em] rounded-[4px] pb-[1px]"
            >
              {char}
            </span>
          );
        ...
        return (
          <span key={i} className={`${t.operatorColor} mx-[1px]`}>
            {char}
          </span>
        );
      }
      if (char === "*") {
        return (
          <span
            key={i}
            className="opacity-50 text-[0.7em] align-top relative top-[0.4em] ml-[2px] font-bold"
          >
            *
          </span>
        );
    ...
    try {
      ...
      if (saved) {
        ...
        return parsed.map((p: Preset) => {
          if (p.id === "tamiflu" && p.name === "Tamiflu")
            return { ...p, name: "Oseltamivir" };
          return p;
        });
      }
    } catch (e) {}
  ...
  const performOperation = (op: string) => {
    ...
    setExpr((prev) => {
      ...
      if (p === "Sai logic toán" || p === "Infinity" || p === "-Infinity")
        return "0" + op;
      if (/[\+\-\×\:]$/.test(p)) {
  ...
  const handleEqual = () => {
    ...
    try {
      const resultNum = precision(
        Function(`'use strict'; return (${calcExpr})`)(),
      );
  ...
  const [tamifluPrompt, setTamifluPrompt] = useState<{
    w: number;
    preset: Preset;
  } | null>(null);
  ...
  const executeTamiflu = (
    ...
    addHistory({
      ...
      expression: [
        `• Cách 1 viên (75mg) pha 15 mL nước`,
        `• Lấy liều: ${note}`,
      ],
      rawExpression: [
        `• Cách 1 viên (75mg) pha 15 mL nước`,
    ...
    setExpr(
      isRoundingEnabled ? formatResult(volume) : formatResult(volume, true),
    );
    setExprIsMl(true);
    setIsResult(true);
  };
  ...
  const renderOpBtn = (op: string) => {
    if (op === "×") {
      return (
        <Btn
          ...
          className={`bg-yellow-400 text-yellow-900 hover:bg-yellow-500 shadow-md font-bold pt-1 border border-yellow-500 text-5xl`}
        >
          ×
        </Btn>
      );
    ...
    return (
      <Btn
        ...
        className={`${t.btnOp} font-light pb-1 text-4xl`}
      >
        {displayOp}
      </Btn>
    );
  ...
  return (
    <div className="fixed inset-0 h-[100dvh] w-full bg-black flex flex-col items-center justify-center font-sans tracking-tight px-0 sm:px-2 pt-0 sm:pt-4 pb-0 sm:pb-4 overflow-hidden select-none">
      {isDesktopOrTablet && (
        <div className="absolute inset-0 z-[200] bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-6 animate-in fade-in duration-300">
          <div className="bg-white rounded-[2rem] p-6 w-full max-w-sm shadow-2xl border border-slate-200 flex flex-col items-center text-center">
            ...
            <h2 className="text-slate-800 text-xl font-black mb-2 leading-tight">
              Thiết bị không tối ưu
            </h2>
            <p className="text-slate-600 mb-6 font-medium text-sm leading-relaxed">
              Ứng dụng SonCal hiện chỉ hỗ trợ thiết bị điện thoại (màn hình dọc)
              để tối ưu về giao diện!
            </p>
          </div>
        </div>
      )}
      ...
      <div
        className={`${t.app} w-full flex-1 max-h-full sm:max-h-[820px] min-h-0 max-w-md rounded-none sm:rounded-[2.5rem] shadow-none sm:shadow-[0_0_20px_rgba(255,255,255,0.05)] flex flex-col overflow-hidden relative transition-colors duration-500 border-0 sm:border border-[#2a2a2a]`}
      >
        {weightWarning && (
          <div className="absolute inset-0 z-[100] bg-slate-900/40 backdrop-blur-md flex items-center justify-center p-6 animate-in fade-in duration-300">
            <div className="bg-white rounded-[2rem] p-6 w-full max-w-sm shadow-2xl border border-slate-200 flex flex-col items-center zoom-in-95">
              <h2 className="text-amber-600 text-2xl font-black mb-2 text-center">
                Cảnh báo
              </h2>
              <p className="text-slate-600 text-center mb-6 font-medium">
                Cân nặng nhập vào là{" "}
                <strong className="text-slate-900">{weightWarning.w} kg</strong>
                .<br />
                Bạn có chắc chắn muốn tính liều trẻ em cho mức cân nặng này?
              </p>
        ...
        {tamifluPrompt && (
          <div className="absolute inset-0 z-[100] bg-slate-900/40 backdrop-blur-md flex items-center justify-center p-6 animate-in fade-in duration-300">
            <div className="bg-white rounded-[2rem] p-6 w-full max-w-sm shadow-2xl border border-slate-200 flex flex-col items-center zoom-in-95">
              <h2 className="text-amber-500 text-2xl font-black mb-2 text-center">
                Độ tuổi của trẻ
              </h2>
              <p className="text-slate-600 text-center mb-6 font-medium">
                Trẻ có cân nặng ≤ 15kg.
                <br />
                Vui lòng chọn độ tuổi tương ứng:
              </p>
        ...
        {showIdleOverlay && (
          <div className="absolute inset-0 z-[100] bg-slate-900/40 backdrop-blur-md flex items-center justify-center p-6 animate-in fade-in duration-300">
            <div className="bg-white rounded-[2rem] p-6 w-full max-w-sm shadow-2xl border border-slate-200 flex flex-col items-center zoom-in-95">
              <h2 className="text-slate-800 text-2xl font-black mb-6 text-center">
                Tiếp tục tính toán?
              </h2>
              <div className="flex gap-4 w-full">
                <button
        ...
        <div className="absolute top-0 inset-x-0 z-30 flex justify-center pointer-events-none pt-[max(env(safe-area-inset-top),0px)]">
          <div className="bg-[#ffcb05] border-b border-x border-[#eab308] rounded-b-[1.5rem] shadow-sm pointer-events-auto px-4 py-1.5 flex items-center justify-center gap-1 w-fit">
            <span className="text-amber-900 font-bold text-[12px] sm:text-[13px] whitespace-nowrap">
              Đang tính liều cho trẻ
            </span>
            <input
              id="weight-input"
              type="number"
          ...
          <div className="flex-1 px-3 sm:px-4 pt-[calc(max(env(safe-area-inset-top),0px)+3.5rem)] pb-3 space-y-3 flex flex-col">
            {history.length === 0 && (
              <div className="m-auto text-center text-slate-400 p-6">
                ...
                <p className="text-[10px] mt-2 max-w-[280px] mx-auto leading-relaxed opacity-60">
                  Máy tính cầm tay này được thiết kế để phục vụ thực hành hàng
                  ngày, không thay thế các quyết định lâm sàng, chỉ sử dụng
                  logic toán học, không sử dụng AI trong tính toán.
                </p>
              </div>
            )}
            {history.length > 0 && (
            ...
            {history.map((item) =>
              item.type === "preset" ? (
                <div
                  key={item.id}
                  className="bg-white p-2.5 rounded-xl shadow-[0_2px_8px_-4px_rgba(0,0,0,0.1)] text-right border-l-[4px] border-emerald-500 flex flex-col gap-0.5 animate-in slide-in-from-bottom-2 fade-in duration-200"
                >
                    ...
                    <div className="flex-shrink-0">
                      {editingLabelId === item.id ? (
                        ...
                        >
                          ...
                          <span className="max-w-[80px] sm:max-w-[120px] truncate leading-none pt-[1px]">
                            {item.label || "Thêm nhãn"}
                          </span>
                        </button>
                      )}
                    </div>
                    <div className="text-emerald-700 text-[10px] font-black uppercase tracking-wide opacity-90">
                      {item.title}
                    </div>
                  </div>
                  <div className="text-slate-500 font-mono text-[12px] border-b border-slate-50 pb-1">
                    {Array.isArray(
                      ...
                      ? (isRoundingEnabled
                          ...
                          <div key={i}>{formatWithThousandSeparator(line)}</div>
                        ))
                  ...
                  <div className="text-slate-900 font-bold text-[17px] leading-tight pt-1">
                    ...
                    {item.volumeNum !== undefined &&
                      getFractionText(String(item.volumeNum)) &&
                      !(
                        isRoundingEnabled
                          ? item.result
                          : item.rawResult || item.result
                  ...
                  {(() => {
                    let displayTotal = isRoundingEnabled
                      ? item.totalVolume
                      : item.rawTotalVolume || item.totalVolume;
                    const dStr = parseFloat(daysStr);
                    if (
                      item.volumeNum &&
                      item.timesPerDay &&
                      ...
                      const newTotalVol = precision(
                        item.volumeNum * item.timesPerDay * dStr,
                      );
                    ...
                    const hasRounded =
                      ...
                      (item.totalVolume || "").includes("*") ||
                      (displayTotal || "").includes("*");
                            ...
                            <div className="text-slate-400 text-[10px] font-medium italic">
                              *{" "}
                              {isRoundingEnabled
                                ...
                                : "Hiển thị số thập phân gốc"}
                            </div>
                            <button
                              onClick={() =>
                                setIsRoundingEnabled(!isRoundingEnabled)
                              }
                              className={`w-7 h-4 rounded-full flex items-center p-0.5 shadow-inner transition-colors ${isRoundingEnabled ? "bg-emerald-500" : "bg-slate-300"}`}
                            >
                              <div
                                className={`w-3 h-3 rounded-full bg-white shadow-sm transition-transform ${isRoundingEnabled ? "translate-x-3" : "translate-x-0"}`}
                              />
                            </button>
                          </div>
                        )}
                ...
                <div
                  key={item.id}
                  className="bg-white p-2.5 rounded-xl shadow-[0_2px_8px_-4px_rgba(0,0,0,0.1)] flex flex-col gap-0.5 border border-slate-100 animate-in slide-in-from-bottom-2 fade-in duration-200"
                >
                  <div className="flex justify-between items-start w-full gap-2 mb-1">
                    <div className="flex-shrink-0">
                      {editingLabelId === item.id ? (
                  ...
                  <div className="text-slate-400 text-[12px] font-mono border-b border-slate-50 pb-1 text-right">
                    {formatWithThousandSeparator(
                      Array.isArray(item.expression)
                        ? (item.expression as any).join(" ")
                        : (item.expression as string),
                    )}
                  </div>
                  <div className="flex flex-col items-end">
                    <span className="text-slate-800 font-bold text-lg font-mono pt-1 leading-tight">
                      {formatWithThousandSeparator(
                        isRoundingEnabled
                          ? item.result
                          : item.rawResult || item.result,
                    ...
                    {getFractionText(
                      ...
                      <span className="text-slate-500 font-bold text-sm -mt-0.5 pointer-events-none">
                        {getFractionText(
                          isRoundingEnabled
                            ? item.result
                            : item.rawResult || item.result,
        ...
        <div className="bg-white shadow-[0_-10px_20px_-10px_rgba(0,0,0,0.05)] z-20 flex-shrink-0 relative">
          ...
          <div className={`transition-all duration-300 flex flex-col w-full`}>
            {/* Presets Bar */}
            <div
              className={`border-y border-slate-100 bg-slate-50 flex overflow-x-auto hide-scrollbar gap-2 px-4 py-2.5`}
            ...
            >
              {presets.map((p) => {
                ...
                let iconColor =
                  "text-slate-300 active:text-blue-500 border-slate-200";

                if (isTamiflu) {
                  btnBg = "bg-amber-100 border-amber-200 text-amber-800";
                  iconColor =
                    "text-amber-400 active:text-amber-600 border-amber-200";
                } else if (p.color) {
                  switch (p.color) {
                    case "red":
                      btnBg = "bg-red-50 border-red-200 text-red-700";
                      iconColor =
                        "text-red-300 active:text-red-500 border-red-200";
                      break;
                    case "amber":
                      btnBg = "bg-amber-50 border-amber-200 text-amber-700";
                      iconColor =
                        "text-amber-300 active:text-amber-500 border-amber-200";
                      break;
                    case "emerald":
                      btnBg =
                        "bg-emerald-50 border-emerald-200 text-emerald-700";
                      iconColor =
                        "text-emerald-300 active:text-emerald-500 border-emerald-200";
                      break;
                    case "sky":
                      btnBg = "bg-sky-50 border-sky-200 text-sky-700";
                      iconColor =
                        "text-sky-300 active:text-sky-500 border-sky-200";
                      break;
                    case "purple":
                      btnBg = "bg-purple-50 border-purple-200 text-purple-700";
                      iconColor =
                        "text-purple-300 active:text-purple-500 border-purple-200";
                      break;
                    case "pink":
                      btnBg = "bg-pink-50 border-pink-200 text-pink-700";
                      iconColor =
                        "text-pink-300 active:text-pink-500 border-pink-200";
                      break;
                  }
                }
                ...
                return (
                  ...
                  >
                    <span
                      ...
                      className="block pr-1 py-0.5 leading-none"
                    >
                      {p.name}
                    </span>
                    {!isTamiflu && (
            ...
            {/* Keypad */}
            <div
              className={`${t.keypad} transition-all duration-500 relative px-3 pt-3 sm:p-4 pb-[max(env(safe-area-inset-bottom),16px)] sm:pb-4`}
            >
              <div
                className={`grid grid-cols-4 transition-all duration-300 gap-[7px] sm:gap-2.5`}
              >
                <Btn
                  onClick={() => clearAll(false)}
                  className={`bg-red-50 text-red-600 font-black hover:bg-red-100 text-2xl col-span-2`}
                >
                  AC
                </Btn>
                <Btn
                  onClick={handleBackspace}
                  className={`${t.btnAction} font-bold`}
                >
                  <Delete className={"w-7 h-7"} />
                </Btn>
                {renderOpBtn(opOrder[0])}

                <Btn onClick={() => inputDigit("7")} className={`${t.btnNum} `}>
                  7
                </Btn>
                <Btn onClick={() => inputDigit("8")} className={`${t.btnNum} `}>
                  8
                </Btn>
                <Btn onClick={() => inputDigit("9")} className={`${t.btnNum} `}>
                  9
                </Btn>
                {renderOpBtn(opOrder[1])}

                <Btn onClick={() => inputDigit("4")} className={`${t.btnNum} `}>
                  4
                </Btn>
                <Btn onClick={() => inputDigit("5")} className={`${t.btnNum} `}>
                  5
                </Btn>
                <Btn onClick={() => inputDigit("6")} className={`${t.btnNum} `}>
                  6
                </Btn>
                {renderOpBtn(opOrder[2])}

                <Btn onClick={() => inputDigit("1")} className={`${t.btnNum} `}>
                  1
                </Btn>
                <Btn onClick={() => inputDigit("2")} className={`${t.btnNum} `}>
                  2
                </Btn>
                <Btn onClick={() => inputDigit("3")} className={`${t.btnNum} `}>
                  3
                </Btn>
                {renderOpBtn(opOrder[3])}

                <Btn onClick={() => inputDigit("0")} className={`${t.btnNum} `}>
                  0
                </Btn>
                <Btn onClick={inputDot} className={`${t.btnNum} pb-3 text-4xl`}>
                  .
                </Btn>
                <Btn
                  onClick={handleEqual}
                  className={`${isPremium ? "bg-gradient-to-b from-amber-400 to-amber-500 text-amber-950 shadow-[0_4px_15px_rgba(245,158,11,0.5)] border-2 border-amber-300" : t.btnEq} font-black rounded-2xl col-span-2 shadow-md text-4xl flex items-center justify-center`}
                >
                  =
                </Btn>
              </div>
      ...
      <SettingsModal
        ...
        secretPin={secretPin}
        onChangePin={() =>
          setAskPinModalType(secretPin ? "change_pin" : "create")
        }
      />
      ...
      {premiumModalOpen && (
        <div className="absolute inset-0 z-[100] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-6 animate-in fade-in duration-300">
          <div className="bg-white rounded-[2rem] p-6 w-full max-w-sm shadow-2xl border border-slate-200 flex flex-col items-center zoom-in-95">
            ...
            <h2 className="text-slate-800 text-2xl font-black mb-2 text-center">
              Nâng cấp Premium
            </h2>
            <p className="text-slate-500 text-sm text-center mb-6 font-medium leading-relaxed">
              Bạn cần nhập mã kích hoạt để thêm{" "}
              <strong className="text-slate-800">thuốc mới</strong> không giới
              hạn!
            </p>
            <div className="relative w-full">
              ...
              />
              <div
                className={`w-full text-center font-mono text-[22px] border rounded-xl px-4 py-3 transition-colors ${premiumError ? "border-red-500 bg-red-50 text-red-600" : premiumCode.length > 0 ? "border-amber-500 ring-2 ring-amber-500/20 text-slate-800" : "border-slate-300 bg-slate-50"}`}
              >
                <div className="flex justify-center items-center tracking-[0.2em] font-black h-7">
                  {premiumCode.length === 0 ? (
                    <span className="text-slate-300 tracking-widest font-normal text-xl">
                      XXX-XXX
                    </span>
                  ) : (
                    premiumCode.slice(0, 3).replace(/./g, "•") +
                    (premiumCode.length > 3
                      ? "-" + premiumCode.slice(3, 6).replace(/./g, "•")
                      : "")
                  )}
                </div>
              </div>
