...
export function PresetModal({
  ...
  const handleSave = () => {
    ...
    if (
      maxDoseMg.trim() !== "" &&
      (isNaN(maxD as number) || (maxD as number) <= 0)
    ) {
    ...
    if (
      bottleVolume.trim() !== "" &&
      (isNaN(bVol as number) || (bVol as number) <= 0)
    ) {
        ...
        {showConfirm && pendingSaveData ? (
          ...
          >
            <div className="px-5 py-4 bg-amber-100 flex items-center justify-center gap-2 border-b border-amber-200">
              ...
              <h2 className="text-[15px] font-bold text-amber-900 tracking-wide uppercase">
                Xác nhận kĩ liều lượng
              </h2>
            </div>
            <div className="p-5 space-y-3">
              <p className="text-amber-800 text-sm font-medium leading-relaxed bg-amber-200/50 p-3 rounded-xl border border-amber-200">
                Vui lòng kiểm tra lại thật kĩ thông tin thuốc bên dưới để đảm
                bảo tính toán an toàn cho bệnh nhân:
              </p>
              <div className="bg-white p-4 rounded-xl border border-amber-200 shadow-sm space-y-2">
                ...
                <div className="flex justify-between border-b border-slate-100 pb-2">
                  <span className="text-slate-500 text-sm">
                    Liều ({doseUnit}/kg/lần):
                  </span>
                  <span className="font-black text-rose-600 text-sm text-right">
                    {pendingSaveData.dosePerKg} {doseUnit}/kg/lần
                  </span>
                </div>
                <div className="flex justify-between border-b border-slate-100 pb-2">
                  <span className="text-slate-500 text-sm">Hàm lượng:</span>
                  <span className="font-bold text-slate-800 text-sm text-right">
                    {pendingSaveData.concentrationMg}
                    {doseUnit} /{" "}
                    {pendingSaveData.isSolid
                      ? "1"
                      : pendingSaveData.concentrationMl}{" "}
                    {pendingSaveData.unit}
                  </span>
                </div>
                {pendingSaveData.maxDoseMg && (
                  <div className="flex justify-between border-b border-slate-100 pb-2">
                    <span className="text-slate-500 text-sm">Liều Max:</span>
                    <span className="font-bold text-rose-600 text-sm text-right">
                      {pendingSaveData.maxDoseMg} {doseUnit}
                    </span>
                  </div>
                )}
                {pendingSaveData.bottleVolume && (
                  <div className="flex justify-between border-b border-slate-100 pb-2">
                    <span className="text-slate-500 text-sm">
                      Thể tích lọ/tuýp/bơm:
                    </span>
                    <span className="font-bold text-slate-800 text-sm text-right">
                      {pendingSaveData.bottleVolume} {pendingSaveData.unit}
                    </span>
                  </div>
                )}
                <div className="flex justify-between border-b border-slate-100 pb-2">
                ...
                {pendingSaveData.note && (
                  <div className="flex justify-between pt-1">
                    <span className="text-slate-500 text-sm w-1/3">
                      Ghi chú:
                    </span>
                    <span className="font-medium text-slate-700 text-sm text-right w-2/3 break-words">
                      {pendingSaveData.note}
                    </span>
                  </div>
                )}
          ...
          >
            <div className="px-5 py-4 flex justify-between items-center border-b border-slate-100">
              <h2 className="text-base font-bold text-slate-800">
                {initialData ? "Sửa thông tin" : "Thêm thông tin"}
              </h2>
              <button
                onClick={onClose}
            ...
            <div className="px-5 py-4 space-y-4 max-h-[72vh] overflow-y-auto custom-scrollbar">
              {/* Tên nút */}
              <div>
                <label className="block text-[10px] tracking-wider uppercase font-bold text-slate-500 mb-1.5">
                  Tên nút (VD: Augmentin)
                </label>
                <input
                  value={name}
              ...
              <div className="bg-slate-50 p-3 rounded-xl border border-slate-200 space-y-2">
                ...
                <div className="flex items-center gap-2 pt-1">
                  ...
                  {isSolid && (
                    <div className="flex-1 min-w-0 text-center font-mono text-sm font-bold text-slate-400 py-1.5 bg-slate-100 rounded-lg border border-slate-200 opacity-50">
                      1
                    </div>
                  )}
                  <input
            ...
            <div className="px-5 py-4 border-t border-slate-100 flex gap-2">
              {initialData && onDelete && (
                <button
                  ...
                  className="px-4 bg-red-50 hover:bg-red-100 text-red-600 text-sm font-bold py-2.5 rounded-xl transition-colors active:scale-95 border border-red-100"
                >
                  Xóa
                </button>
              )}
              <button
                onClick={handleSave}
                className="flex-1 bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold py-2.5 rounded-xl shadow-md shadow-blue-200/50 transition-all active:scale-95"
              >
                Lưu
              </button>
            </div>
