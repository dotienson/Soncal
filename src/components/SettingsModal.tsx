    ...
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl w-full max-w-sm max-h-[85vh] overflow-y-auto custom-scrollbar shadow-2xl animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="p-5 space-y-6">
          ...
          <div>
            <h3 className="font-semibold text-slate-700 mb-3 text-[14px] flex items-center gap-2 uppercase tracking-wide">
              <LayoutGrid className="w-4 h-4 text-slate-400" /> Bố cục phím toán
              tử
              {!isPremium && (
                <Crown
                  className="w-3.5 h-3.5 text-amber-500 ml-1"
                  fill="currentColor"
                />
              )}
            </h3>
            <div className="relative">
              ...
              <div
                className={`flex flex-row gap-2 ${!isPremium ? "opacity-60" : ""}`}
              >
                {opOrder.map((op, idx) => (
                  <div
                    key={op}
          ...
          <div>
            <h3 className="font-semibold text-slate-700 mb-3 text-[14px] flex items-center gap-2 uppercase tracking-wide">
              <Crown className="w-4 h-4 text-slate-400" /> Bảo mật tự động tính
              (PIN)
              {!isPremium && (
                <Crown
                  className="w-3.5 h-3.5 text-amber-500 ml-1"
            ...
            <div className="relative bg-slate-50 p-4 rounded-xl border border-slate-200">
              ...
              <div
                className={`flex items-center justify-between ${!isPremium ? "opacity-60" : ""}`}
              >
                <div>
                  <p className="font-bold text-slate-800 text-sm">Số bảo mật</p>
                  <p className="text-xs text-slate-500 mt-1 max-w-[200px]">
                    Tránh ấn sửa/xoá vô tình các liều thuốc.
                  </p>
                </div>
                <button
                  onClick={() => {
          ...
          <div>
            <h3 className="font-semibold text-slate-700 mb-3 text-[14px] flex items-center gap-2 uppercase tracking-wide">
              <Download className="w-4 h-4 text-slate-400" /> App cho iOS (Add
              to Home Screen)
            </h3>
            <div className="bg-blue-50/50 text-slate-700 p-4 rounded-xl text-[13px] leading-relaxed border border-blue-100 shadow-inner">
              <ul className="list-decimal pl-4 space-y-2">
                <li>
                  ...
                  trên trình duyệt <strong>Safari</strong>.
                </li>
                <li>
                  Chạm vào biểu tượng <strong>Share</strong> (Chia sẻ) ở thanh
                  công cụ dưới cùng.
                </li>
                <li>
                  Cuộn xuống và chọn <strong>Add to Home Screen</strong> (Thêm
                  vào màn hình chính).
                </li>
                <li>
                  Bấm <strong>Add</strong> (Thêm) ở góc trên bên phải.
                </li>
              </ul>
            </div>
          </div>
          ...
          <div>
            ...
            {showArch && (
              <div className="bg-slate-900 text-slate-300 p-4 rounded-xl text-xs leading-relaxed space-y-4 font-mono shadow-inner border border-slate-700 mt-2 animate-in fade-in slide-in-from-top-2 duration-200">
                <p>
                  <strong className="text-sky-400 block mb-1 uppercase text-[10px] tracking-wider">
                    Engine Execution
                  </strong>
                  Chạy trực tiếp qua native hardware-accelerated JavaScript
                  engines (V8/Nitro). Native compilation ở lớp thực thi
                  (execution layer) mang lại độ trễ gần như bằng 0 (near-zero
                  latency), không bị overhead của network round-trip.
                </p>
                <p>
                  <strong className="text-sky-400 block mb-1 uppercase text-[10px] tracking-wider">
                    Mathematical Reliability
                  </strong>
                  Strict algebraic precedence parsing (PEMDAS). Trình tạo AST
                  (AST generation) xử lý tính toán với độ chính xác tuyệt đối
                  (absolute determinism), hoàn toàn tin cậy cho tính toán lâm
                  sàng.
                </p>
                <p>
                  <strong className="text-sky-400 block mb-1 uppercase text-[10px] tracking-wider">
                  ...
                  Sử dụng 64-bit precision (IEEE 754). Tích hợp sẵn hàm
                  differential noise reduction <code>(toPrecision(12))</code>{" "}
                  giúp loại bỏ lỗi nhị phân (binary artifacting - ví dụ: 0.1 +
                  0.2), đảm bảo scale thập phân hoàn hảo đối với liều chia nhỏ.
                </p>
              </div>
            )}
          </div>
          ...
          <div>
            ...
            <div className="bg-amber-50 text-slate-800 p-4 rounded-xl text-sm leading-relaxed border border-amber-100 shadow-inner flex flex-col items-center">
              <p className="mb-3 text-center text-slate-600 font-medium">
                Nếu bạn thấy công cụ hữu ích, có thể ủng hộ tác giả qua kênh bên
                dưới:
              </p>
              <div className="bg-white p-3 rounded-lg border border-amber-200 block w-full text-center shadow-sm relative">
                <div className="flex items-center justify-center gap-2">
                  <p className="font-bold text-xl text-slate-800 tracking-wider">
                    120274848
                  </p>
                  <button
                    onClick={handleCopy}
                ...
                <p className="text-slate-500 font-medium text-xs mt-1">
                  VPBank <span className="mx-1">•</span> Do Tien Son
                </p>
              </div>
            </div>
          </div>
